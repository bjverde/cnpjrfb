...
class QualsDAO 
{
    ...
    private static $sqlBasicSelect = 'select
                                      codigo
                                     ,descricao
                                     from dados_rfb.quals ';

    private $tpdo = null;
    private $repositoryName = 'quals'; //Nome da Classe do tipo Active Record no diretorio /app/model/maindatabase
    ...
    public function selectCount( $where=null )
    {
        ...
        $sql = 'select count(codigo) as qtd from dados_rfb.quals';
    ...
    //--------------------------------------------------------------------------------
    public function insert( QualsVO $objVo )
    {
        $values = array(  $objVo->getDescricao() 
                        );
        $sql = 'insert into dados_rfb.quals(
                                 descricao
                                ) values (?)';
    ...
    //--------------------------------------------------------------------------------
    public function update ( QualsVO $objVo )
    {
        $values = array( $objVo->getDescricao()
                        ,$objVo->getCodigo() );
        $sql = 'update dados_rfb.quals set 
                                 descricao = ?
                                where codigo = ?';
        $result = $this->tpdo->executeSql($sql, $values);
        return intval($result);
    ...
    public function delete( $id )
    {
        FormDinHelper::validateIdIsNumeric($id,__METHOD__,__LINE__);
        $values = array($id);
        $sql = 'delete from dados_rfb.quals where codigo = ?';
        $result = $this->tpdo->executeSql($sql, $values);
        return $result;
    }
    //--------------------------------------------------------------------------------
